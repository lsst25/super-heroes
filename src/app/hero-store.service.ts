...
export class HeroStoreService {
  ...
  get selectedPowerupsIds(): number[] {
    return this.selectedPowerups.map((powerup: Powerup) => powerup.id);
  }

  selectPowerup(id: number): void {
    this.selectedPowerups.push(
      this.powerups.find((powerup: Powerup) => powerup.id === id) as Powerup
    );
  }

  unselectPowerup(id: number): void {
    this.selectedPowerups = this.selectedPowerups.filter(
      (powerup: Powerup) => powerup.id !== id
    );
  }

  usePowerup(id: number): void {
    const powerupToUpdate = this.powerups.find((powerup: Powerup) => powerup.id === id);
    if (powerupToUpdate) {
  ...
  setSelectedHeroes(): void {
    this.selectedHeroes =
      this.storageService.getFromStorage('selectedHeroes') || [];
    this.lastSelectedHero = this.storageService.getFromStorage(
      'lastSelectedHero' || undefined
    );
  }
